...
export function PaymentPage() {
    ...
    // Upload Bukti Pembayaran - FIXED VERSION
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        const submitData = {
            ...
            down_payment: paymentMethod === 'credit' ? creditCalculation.downPaymentAmount : orderTotal,
        ...
        try {
            ...
            if (data.success) {
                ...
                if (paymentMethod === 'cash') {
                    toast.success('Pembelian Cash berhasil! Sales akan menghubungi Anda.');
                    setShowSuccess(true);
                    if (fromCart) clearCart();
                } else {
                    setPaymentStep('payment');
                    toast.success('Order Kredit berhasil dibuat! Lanjutkan pembayaran DP.');
                }
            } else {
...
- Hubungi customer service jika ada pertanyaan
    ...
    // Komponen Upload Bukti Pembayaran - FIXED
    ...
    const PaymentMethods = () => {
                        ...
                        <div className="bg-blue-50 p-4 rounded border border-blue-200">
                            ...
                            <div className="mt-4 text-sm text-blue-700">
                                ...
                                <ol className="list-decimal list-inside space-y-1 mt-2">
                                    ...
                                    <li>Jumlah transfer: <strong>{formatPrice(paymentMethod === 'credit' ? creditCalculation.downPaymentAmount : orderTotal)}</strong></li>
        ...
        return (
            <div className="border rounded-lg p-6">
                <h3 className="text-lg font-semibold mb-4">Pilih Metode Pembayaran Manual</h3>
                ...
                <div className="mb-8 text-center">
                    <h1 className="text-4xl mb-2">
                        {paymentStep === 'form' ? 'Form Pembelian Motor' : 
                         paymentStep === 'payment' ? 'Pembayaran DP/Total' : 
                         'Upload Bukti Pembayaran'}
                    </h1>
                    <p className="text-gray-600">
                        {paymentStep === 'form' ? 'Lengkapi data diri untuk proses pembelian' : 
                         paymentStep === 'payment' ? `Silakan pilih metode pembayaran untuk Down Payment/Total (${paymentMethod === 'credit' ? 'Kredit' : 'Cash'})` :
                                ...
                                <div className="space-y-4">
                                    ...
                                    <div className="border-t pt-4">
                                        <div className="flex justify-between mb-2">
                                            <span className="text-gray-600">Harga Motor</span>
                                            <span>{formatPrice(orderTotal)}</span>
                                        </div>
                                        <div className="flex justify-between mb-2">
                                            <span className="text-gray-600">Biaya Admin + STNK</span>
                                            <span>{formatPrice(1500000)}</span>
                                        </div>
                                        {paymentMethod === 'cash' && (
                                            <div className="flex justify-between mb-2 text-green-600">
                                                <span className="font-semibold">Diskon Cash</span>
                                                <span className="font-semibold">-{formatPrice(2000000)}</span>
                                            </div>
                                        )}
                                        <div className="flex justify-between text-lg font-bold border-t pt-2 mt-2">
                                            <span>Total Akhir</span>
                                            <span className="text-red-600">
                                                {formatPrice(orderTotal + 1500000 - (paymentMethod === 'cash' ? 2000000 : 0))}
                                            </span>
                                        </div>
                                    </div>
                                    ...
                                    <form onSubmit={handleSubmit} className="space-y-6">
                                        ...
                                        >
                                            {isSubmitting ? (
                                                ...
                                                `Lanjut ke Pembayaran ${paymentMethod === 'credit' ? 'DP' : 'Total'}`
                                            )}
                                        </Button>
                                    </form>
                                </CardContent>
                            ...
                            <Card>
                                <CardHeader>
                                    <CardTitle className="flex items-center gap-2">
                                        ...
                                        Langkah 2: Pembayaran {paymentMethod === 'credit' ? 'Down Payment' : 'Total'}
                                    </CardTitle>
                                </CardHeader>
                                <CardContent>
                                    <PaymentMethods />
